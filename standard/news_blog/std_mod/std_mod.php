...
class StandardModule {
            ...
            switch($_POST['action']) {
                ...
                case 'row_number_increase': {
                    ...
                    if($lock_current = mysql_fetch_assoc($rs_current)) { //current record (need to be moved up)
                        ...
                        if (($this->level > 0) && $this->currentArea->dbReference)
  ...
  <div onmousedown="getPos(event)" id="splitterBar" >
    ...
    from `".mysql_real_escape_string(DB_PREF.mysql_real_escape_string($area->dbTable))."` where 1 ";
        if ($parentId && $this->currentArea->dbReference)
        $sql .= " and `".mysql_real_escape_string($area->dbReference)."` = '".mysql_real_escape_string($parentId)."' ";
        if($area->whereCondition)
...
<body> <!-- display loading until page is loaded-->
      ...
      <!-- display loading until page is loaded-->		
		...
		<script src="'.BASE_URL.PLUGIN_DIR.'standard/news_blog/public/manager.js" ></script>
...
<script type="text/javascript">
  ...
  //]]>
        ...
        if(method_exists($area, 'allowDelete')) {
            ...
            if ($child != null) {
                if ($child->dbReference) {
                    ...
                    $sql = " select `".mysql_real_escape_string($child->dbPrimaryKey)."` as 'key' from `".mysql_real_escape_string(DB_PREF.$child->dbTable)."` where 1 ";
                }
                $rs = mysql_query($sql);
                if ($rs) {
    ...
    function printTabs() {
        ...
        $answer .= '
			<li onclick="document.getElementById(\'std_mod_new_popup_body\').style.height=(150) + \'px\'; document.getElementById(\'std_mod_new_popup\').style.display = \'block\';"><span>'.$parametersMod->getValue('developer','std_mod','admin_translations','new').'</span></li>';
